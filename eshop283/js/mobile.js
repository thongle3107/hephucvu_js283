...
const locGia=(tag)=>{
    ...
    xuatSanphamDienthoai(tmp,thLstMobile)
}
...
const keyCode=(event)=>{
    if (event.keyCode == 13) {
        ...
        xuatSanphamDienthoai(dsTim,thLstMobile)
    }
}
...
const sxGia=(tag)=>{
    ...
    xuatSanphamDienthoai(ds,thLstMobile)
}
...
const xuatSanphamtheoNhom=(maNhom)=>{
    ...
    if(maNhom=="ALL"){
        ...
        ds=dsDienthoai.filter(x=>x.Nhom.Ma_so.toLowerCase()==maNhom.toLowerCase());
    }
...
const xuatSanphamDienthoai = (ds = [], tag) => {
    let html = ``;
    ds.forEach(sp => {
        html += `
        <div class="col mb-5">
            <div class="card h-100">
                ...
                <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                    ...
                    <a class="btn btn-sm btn-outline-dark mt-auto" href="javaScript:void(0)" onclick="addCart('${sp.Ma_so}','3')">Add to cart</a>
                    </div>
                </div>
            </div>
        </div>
